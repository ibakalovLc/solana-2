...
pub fn send_wanted_tokens_to_maker(ctx: &Context<TakeOffer>) -> Result<()> {
    ...
    msg!("Transferring tokens from: {:?}", ctx.accounts.taker_token_account_b.key());

    Ok(())
}

pub fn withdraw_tokens_and_close_vault(ctx: Context<TakeOffer>) -> Result<()> {
    let account_pub_key = ctx.accounts.maker.to_account_info().key();

    let seeds = &[
        b"offer",
        account_pub_key.as_ref(),
